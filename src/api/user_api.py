...
def get_user_by_phone(
    ...
    name = user_service.validate_target_name(phone_number)
    if not name:
...
@router.get("/me")
def user_profile(current_user: CurrentUserDep) -> dict[str, bool | Any]:
    profile = user_service.validate_user_profile(current_user.user_id)
    if not profile:
...
@router.get("/owner")
def get_owner_name(current_user: CurrentUserDep) -> dict[str, bool | Any]:
    name = user_service.validate_owner_name(current_user.user_id)
    if not name:
