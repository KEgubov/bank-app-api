...
class ContactService:
    ...
    @staticmethod
    async def validate_find_user_by_phone_number(
        phone_number: str,
    ) -> list[ContactDTO] | None:
        found_user = await contact_repository.find_user_by_phone_number(phone_number)
        if found_user:
            result_dto = [
    ...
    @staticmethod
    async def validate_all_contacts_from_user(user_id: int) -> list[UserContactRelDTO] | None:
        all_contacts = await contact_repository.get_all_contacts_from_user(user_id)
        if all_contacts:
            result_dto = [
    ...
    @staticmethod
    async def response_delete_contact(contact_id: int) -> bool:
        return await contact_repository.delete_contact_in_db(contact_id)
