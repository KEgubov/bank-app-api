...
class UserService:
    ...
    @staticmethod
    async def validate_user(user: UserAddDTO) -> list[UserDTO] | None:
    ...
    async def validate_target_name(phone_number: str) -> list[FirstLastSuperNameDTO] | None:
        target_name = await user_repository.get_target_name(phone_number)
        if target_name:
            result_dto = [
    ...
    @staticmethod
    async def validate_user_profile(user_id: int) -> list[UserDTO] | None:
    ...
    @staticmethod
    async def validate_owner_name(user_id: int) -> list[FirstLastSuperNameDTO] | None:
        model_owner = await user_repository.get_find_owner_name(user_id)
        if model_owner:
            result_dto = [
                FirstLastSuperNameDTO.model_validate(row, from_attributes=True)
