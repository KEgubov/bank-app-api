...
class CardService:
    ...
    @staticmethod
    def validate_card_data(card: CardAddDTO) -> list[CardDTO] | None:
        model_card = Card(**card.model_dump())
        added_card = card_repository.create_card_in_db(model_card)
        if added_card:
    ...
    @staticmethod
    def validate_card_number(card_number: str) -> list[CardNumberDTO] | None:
        card_number = card_repository.get_card_number(card_number)
        if card_number:
            result_dto = [
    ...
    @staticmethod
    def validate_all_cards_from_account(
        account_id: int,
    ) -> list[AccountCardRelDTO] | None:
        all_cards = card_repository.get_all_cards_from_account(account_id)
        if all_cards:
            result_dto = [
    ...
    @staticmethod
    def response_find_card(card_number: str) -> bool:
        return card_repository.find_card(card_number)
